...
public class Particle_Tracking_Division implements PlugIn, DialogListener {
	...
	public void run(String arg) {
		...
		dlg.addNumericField("Division proximity [pixel]", 25, 1);
		...
		double division = dlg.getNextNumber();
		...
		for (int t = 1; t <= nt; t++) {
			...
			checkDivision(nonMatchedSpots, division);
			for (Spot spot : nonMatchedSpots)
				trajectories.add(new Trajectory(spot));
		}
	...
	public void checkDivision(ArrayList<Spot> spots, double division) {
		for (Spot spot : spots) {
			for (Trajectory trajectory : trajectories) {
				for (int i = 4; i < trajectory.size(); i++) {
					Spot mother = trajectory.get(i);
					if (mother.t == spot.t - 1) {
						if (mother.distance(spot) < division) {
							spot.daughterOf = trajectory;
							Roi roi = new OvalRoi(mother.x - division / 2, mother.y - division / 2, division, division);
							roi.setStrokeColor(Color.YELLOW);
							roi.setStrokeWidth(1);
							roi.setPosition(mother.t);
							overlay.add(roi);
						}
					}
				}
